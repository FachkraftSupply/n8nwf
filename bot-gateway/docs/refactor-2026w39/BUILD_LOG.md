...
## WP4 — Admin v2 pilot — 2026-09-25T19:29:23Z

- Staging workflow: `Admin v2 - Hệ thống (STAGING)` (`bauK573MU18oRzKP`), folder `REFACTOR 2026-W39 (staging)` (`ZdlLC9utIKkjLvhv`), project `5cL5BKorhKAQ2ONI`. Final versionId `4851cb58-ede4-4fb8-b323-3dadad9d2b82`. `active: false`, `activeVersionId: null` (never active/published — pilot is called by pinned-envelope test only, per PLAN §6 WP4 point 5). No `errorWorkflow` set (staging, per spec). `settings.executionOrder: "v1"` (matches v1).
- Source (read-only, cloned from): `Telebot Admin System (System Bot dedicated)` `eWtu7Qs85Hes0HuP` @ `activeVersionId` `53d44baa-8e4b-4dd4-8f78-fbcd970fe46f`. Confirmed **unchanged** both before (02:11 VN / Architect) and after (re-checked at end of build) this build — `versionId == activeVersionId == 53d44baa-...` both times, `updatedAt` constant at `2026-09-23T04:50:57.468Z`. Only `get_workflow_details` was ever called on this workflow — zero mutating calls.
- Build method: fetched full v1 JSON, BFS-traced the connection graph from `Check Admin (Errors)`/`(Version)`/`(Token)`/`(VPS)`/`(VPS Extra)` gates downstream to identify the exact node set per route (script in scratchpad), extracted each target node's full definition (id/parameters/credentials/position/typeVersion/settings), applied the `$('Phân tích lệnh')`→`$('Execute Workflow Trigger')` and `$('⚙️ Config').first().json.botUsername`→`$('Execute Workflow Trigger').first().json.config.botUsername` string rewrites to the serialized JSON *before* calling `addNode` (avoids the RULES #16 removeNode+addNode dance entirely since these were new nodes, never previously added). Applied via 2 `update_workflow` calls on a workflow created with `create_workflow_from_code` (1-node manual-trigger placeholder):
  - Batch 1: 33 `addNode` (28 copied from v1 + 5 new: `Execute Workflow Trigger`, `Là vps_cancel?`, `Là Admin?`, `Route`, `Là Lỗi Route?`) + 7 `setNodeSettings` (nodes carrying `onError`/`alwaysOutputData` in v1) — 40 ops, `autoAssignedCredentials: []`. Immediately re-read full workflow and confirmed all 5 HTTP-Request-node credentials (DeepSeek, OpenRouter, VPS Info, Container Info, Container Restart) and all Telegram/Postgres credentials landed correctly at first attempt (RULES #25) — the response's boilerplate "credentials must be configured manually" note for HTTP nodes was a false alarm; `get_workflow_details` confirmed every one present and correct.
  - Batch 2: `removeNode` of the placeholder trigger + 35 `addConnection` (all with explicit `sourceIndex`/`targetIndex` per RULES #13) — 36 ops, `autoAssignedCredentials: []`, **0 `validationWarnings`**.
- Node list copied verbatim from v1 (28, same `id` as v1, RULES #25):
  `Switch (Error Actions)`, `Errors: Query Recent`, `Errors: Mark Reported`, `Errors: Build Report (Logs)`, `Errors: Build Report (Now)`, `Lichsu: Send`, `Reply Không Có Quyền (Errors)`, `Query Changelog`, `Build Version Text`, `Send Version Text`, `Call DeepSeek Balance`, `Call OpenRouter Key Info`, `Merge Token Results`, `Build Token Stats`, `Send Token Stats`, `Call VPS Info`, `Build VPS Stats`, `Send VPS Stats`, `Reply Không Có Quyền (VPS)`, `Call Container Info`, `Call Container Restart`, `Container Info OK?`, `Build Restart Result`, `Build Container Detail`, `Reply Container Info Failed`, `Send Restart Result`, `Send Container Detail`, `Reply VPS Cancelled`.
- New nodes (5, not in v1): `Execute Workflow Trigger` (`n8n-nodes-base.executeWorkflowTrigger` v1.2, `inputSource: "passthrough"` per spec point 1), `Là vps_cancel?` (IF, bypass gate), `Là Admin?` (IF, combined gate replacing the 5 `Check Admin (...)`), `Route` (Switch v3.4, 7 cases, `fallbackOutput: "none"`), `Là Lỗi Route?` (IF, picks which not-admin reply to send).
- **Design decisions made (documented per spec, none blocking):**
  1. **F11 gate merge**: 1 IF `Là Admin?` (`chatId === config.ADMIN_CHAT_ID`) replaces `Check Admin (Errors)`, `(Version)`, `(Token)`, `(VPS)`, `(VPS Extra)` — all 5 had byte-identical conditions in v1 (verified by diff), so no ambiguity.
  2. **REG-VPS-05 parity (explicit spec requirement)**: `vps_cancel` bypasses the admin gate entirely — confirmed in v1 the Switch's `vps_cancel` output (index 25) connects directly to `Reply VPS Cancelled` with **no** `Check Admin` node in between. Replicated with `Là vps_cancel?` IF placed *before* `Là Admin?`: true → `Reply VPS Cancelled` (no admin check), false → `Là Admin?`.
  3. **Not-admin reply node consolidation**: v1 has 4 "Reply Không Có Quyền" nodes for this domain — `(Errors)` text `"⚠️ Bạn không có quyền thực hiện lệnh này."` (distinct), and `(Token)`/`(Version)`/`(VPS)` all sharing byte-identical text `"🚫 Bạn không có quyền dùng lệnh này."` (v1 already reuses `(VPS)` for both `Check Admin (VPS)` and `(VPS Extra)` false-branches — precedent for sharing). Kept only 2 nodes: `Reply Không Có Quyền (Errors)` (copied from v1's own node, exact id) for routes `error_logs`/`error_log_now`, and `Reply Không Có Quyền (VPS)` (copied from v1's own node, exact id) shared for `token`/`version`/`vps`/`vps_container`/`vps_restart`. Did **not** copy v1's separate `Reply Không Có Quyền (Token)` / `(Version)` nodes (would have been byte-identical duplicates). New node `Là Lỗi Route?` (IF, `route === 'error_logs' OR route === 'error_log_now'`) on the not-admin branch picks between the two. This satisfies spec's explicit option "1 node chung nếu text v1 giống nhau."
  4. **`Route VPS Extra` (v1 switch, id `cc4b5fd2-...`) NOT copied**: in v1 it existed only because `Check Admin (VPS Extra)` was one shared gate for both `vps_container` and `vps_restart`, requiring a second switch downstream to re-split by route. In v2 the top-level `Route` switch already dispatches `vps_container` and `vps_restart` as separate cases directly to `Call Container Info` / `Call Container Restart` respectively — `Route VPS Extra`'s function is fully subsumed, keeping it would have been a redundant re-check of the same `route` field with no behavior difference for either admin or non-admin users. Not an Open Question: this is a direct, unambiguous consequence of spec point 3 ("Switch theo route tới đầu nhánh tương ứng").
- **References rewritten** (grepped every `$('...')` in the 28 copied nodes' `parameters`, before and after — RULES #5/#10, spec point 4's "điểm rủi ro cao nhất"):

  | Node | Old reference | New reference |
  |---|---|---|
  | `Switch (Error Actions)` (both rules) | `$('Phân tích lệnh').first().json.route` | `$('Execute Workflow Trigger').first().json.route` |
  | `Errors: Build Report (Logs)` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Errors: Build Report (Now)` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Reply Không Có Quyền (Errors)` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Build Version Text` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Build Token Stats` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Build VPS Stats` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Build VPS Stats` | `$('⚙️ Config').first().json.botUsername` | `$('Execute Workflow Trigger').first().json.config.botUsername` |
  | `Reply Không Có Quyền (VPS)` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Call Container Info` | `$('Phân tích lệnh').first().json.params` | `$('Execute Workflow Trigger').first().json.params` |
  | `Call Container Restart` | `$('Phân tích lệnh').first().json.params` | `$('Execute Workflow Trigger').first().json.params` |
  | `Build Restart Result` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Build Container Detail` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Reply Container Info Failed` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |
  | `Reply VPS Cancelled` | `$('Phân tích lệnh').first().json.chatId` | `$('Execute Workflow Trigger').first().json.chatId` |

  15 total occurrences rewritten (14× `$('Phân tích lệnh')`, 1× `$('⚙️ Config')`), all in the 28 copied nodes' `parameters` — confirmed via `grep`/regex over the serialized JSON both before the rewrite (found exactly these 15, no other context — no false positives from prose text inside the two `claudePrompt` template-literal blocks in `Errors: Build Report (Logs)`/`(Now)`, which is untrusted data quoted verbatim from v1, unrelated to this rewrite) and after (0 remaining occurrences of either string, verified with `assert` in the build script). References to `Build Token Stats`, `Build VPS Stats`, `Call Container Restart`, `Call Container Info`, `Build Restart Result`, `Build Container Detail` from `Send Token Stats`/`Send VPS Stats`/`Send Restart Result`/`Send Container Detail`/`Build Restart Result`/`Build Container Detail` were **left unchanged** — those target nodes are copied into the same sub-workflow, so the references remain valid without rewriting.
- **Static reference-integrity check (script)**: extracted every `$('X')` occurring in any of the 33 nodes' `parameters` from the *final* re-fetched workflow JSON and confirmed **all resolve to a node name that exists in this sub-workflow** — 0 dangling references.
- **Clone-parity check (script)**: for all 28 copied nodes, compared final v2 JSON against v1 JSON: `id` identical ✅, `type`/`typeVersion` identical ✅, `credentials` byte-identical ✅, node `settings` (`onError`/`alwaysOutputData`/`retryOnFail`/`maxTries`/`waitBetweenTries`) identical ✅, `parameters` identical to v1 **except exactly** the 15 rewrites listed above (verified by applying the same string substitution to v1's params and comparing to v2's params — 0 unexpected diffs across all 28 nodes) ✅. Internal connections among the 28 copied nodes: 19/19 edges (including `sourceIndex`/`targetIndex`, e.g. `Call DeepSeek Balance→Merge Token Results` targetIndex 1, `Call OpenRouter Key Info→Merge Token Results` targetIndex 0) byte-identical to v1, 0 missing, 0 extra.
- Credentials (RULES #23): every Telegram node uses `Telegram System Bot` (`zSZ6vVapow5LNpFT`) — same as v1, confirmed by the clone-parity check above (all 12 Telegram nodes). Postgres nodes use `Postgres account` (`GwUFREmcXzXXj5mZ`) — matches PLAN §3. HTTP nodes: `Call VPS Info`/`Call Container Info`/`Call Container Restart` use `X-Auth-Token` (`Ri6Y4WtEuLgl9ixb`); `Call DeepSeek Balance` uses `DeepSeek account` (`F7tLItIIVtzpZGqS`, not separately listed in PLAN §3 but present in v1 and copied verbatim — flagged below); `Call OpenRouter Key Info` uses `OpenRouter account` (`BOzvluQs5DGXl9Yr`) — matches PLAN §3.
- Verified by re-read: `get_workflow_details` (full) after each of the 2 `update_workflow` calls; final workflow has exactly 33 nodes (28 copied + 5 new), `active: false`, `activeVersionId: null`, no `errorWorkflow`, `executionOrder: v1`. Confirmed `Là vps_cancel?` → `Reply VPS Cancelled` wired with no admin check in between (REG-VPS-05 parity intact). `Route` switch's 7 cases wired exactly to the intended heads, including `token` case fanning out to both `Call DeepSeek Balance` and `Call OpenRouter Key Info` (matching v1's `Check Admin (Token)` true-branch fan-out).
- Open questions for architect: none blocking.
  - Non-blocking note: `DeepSeek account` credential (`F7tLItIIVtzpZGqS`) is used by `Call DeepSeek Balance` (copied verbatim from v1) but is not listed in PLAN §3's credential table — likely just an omission in that table (it predates WP4), not a WP4 decision; flagging so the auditor can confirm it's an existing, intentional v1 credential and not something to change.
  - Non-blocking note: per architect instruction and PLAN §6 WP4 point 5, no router v2 was built for Admin — this sub-workflow can only be exercised via `prepare_workflow_pin_data`/`test_workflow` with a hand-built envelope pinned at `Execute Workflow Trigger` (contract: `{ route, params, taskId, chatId, messageId, isCallback, originalData, config: { ADMIN_CHAT_ID, botUsername } }`), per PLAN §7.6 ADM-01..08. Not run in this build (tester's job).
