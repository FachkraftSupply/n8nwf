...
| `/error_logs` (xem) + `/error_log_now` (xem + đánh dấu đã xử lý) | Xem nhanh lỗi tuần qua ngay lập tức, kèm thống kê ✅ đã sửa/🔓 còn mở, kèm sẵn 1 prompt copy thẳng vào Claude Code để sửa (prompt giờ chỉ Claude tự tra + tự ghi lại "fix knowledge base" qua `GW Error Knowledge`) | Query `gateway.error_logs` (kèm `id` từng dòng + CTE thống kê fixed/open/total), `/error_log_now` thêm `UPDATE status='reported'` | 2026-09-09, nâng cấp fix knowledge base 11/09/2026 |
...
| **`/token`** | Xem usage/số dư đang dùng bên OpenRouter (đã dùng/hạn mức/còn lại) và DeepSeek (số dư USD) | 2 HTTP Request song song (`predefinedCredentialType`, dùng credential có sẵn) → gộp qua node Merge → dựng text, chỉ admin (`Check Admin (Token)`) | 2026-09-12 |
...
| **`GW Error Knowledge`** (`GSz6ZluGT5jCgdEc`) | "Fix knowledge base" cho lỗi n8n: webhook POST nhận `{action:"search", keyword}` để tra lỗi tương tự ĐÃ TỪNG được sửa (đọc `fix_description` đã lưu), hoặc `{action:"log_fix", id, fixDescription, fixedBy}` để ghi lại cách sửa vào `gateway.error_logs` sau khi 1 lỗi được fix xong — mục đích để agent (Claude Code) phiên SAU đọc lại được cách đã sửa lỗi tương tự, không phải debug lại từ đầu. Gọi qua n8n MCP `execute_workflow` (executionMode, triggerNodeName:"From Webhook") hoặc HTTP POST thẳng `https://n8n.toididuhoc.net/webhook/gw-error-knowledge` — KHÔNG có xác thực (chấp nhận rủi ro thấp vì chỉ chứa mô tả lỗi/cách sửa nội bộ, không PII) | — (không gửi Telegram, gọi trực tiếp bởi agent) | 2026-09-11, tạo mới |
